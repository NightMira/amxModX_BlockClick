...
get_define_value() {
    ...
    grep -E "^#define[[:space:]]+$define_name+[[:space:]]" "$file" 2>/dev/null | head -1 | awk -F '"' '{print $2}'
}
...
uses_api_version() {
    grep -E "register_plugin\([^,]+,API_VERSION+,[^,]" "$1" >/dev/null 2>&1 ||
    grep -E "API_VERSION" "$1" >/dev/null 2>&1
}
...
API_VERSION=$(get_define_value "$VERSION_FILE" "API_VERSION")
...
compile_plugin() {
    ...
    if [ -z "$plugin_version" ] && ! uses_api_version "$sma_file"; then
        warnings+="VERSION "; local_warnings=$((local_warnings + 1)); plugin_version="";
    elif [ -z "$plugin_version" ] && uses_api_version "$sma_file"; then
        plugin_version="$API_VERSION"
        echo "📝 Using API_VERSION: $plugin_version" >> "$LOG_FILE"
    fi
